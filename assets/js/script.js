...
$(document).click(function(click) {
    ...
    if(!target.hasClass("item") && !target.hasClass("optionsButton") && !target.hasClass("")) {
        hideOptionsMenu();
    }
});
...
function removeFromPlaylist(button, playlistId) {
    ...
    .done(function(error) {
        //.done is prefered - look into "defferreds"
        if(error != "") {
            alert(error);
            return;
        }
...
function updateVolumeProgressBar(audio) {
    var volume = audio.volume * 100;    //volume is between 0 & 100
...
function Audio() {
    ...
    this.audio = document.createElement('audio');   //html element which is an 'audio' object

    this.audio.addEventListener("ended", function() {
        nextSong();
    });

    //update remaining time when able to play a song
    ...
    //set the track given source track
